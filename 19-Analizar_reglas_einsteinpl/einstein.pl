...
% Se genera una posible solucion que no sea contradictoria y despues se validan todos los parametros %
solucion(Solucion) :-
    posibleVecindario(Solucion),
    validarSolucion(Solucion).

% Se genera una solucion con 5 casas, y se manda a la funcion de validar vecindario %
...
% Se genera una casa con los posibles parametros y se manda a validar que no sea contradictoria %
posibleCasa(Casa) :-
    altura(Altura),
    color(Color),
...
% Validaciones %
% Se valida que una casa cumpla con las restricciones conocidas propuestas en las 15 oraciones iniciales
validarCasa(Casa) :- forall(restriccionCasa(RestriccionCasa), cumple(RestriccionCasa, Casa)).

% Se valida que el orden de las casas cumpla con las restricciones conocidas propuestas en las 15 oraciones iniciales % 
validarVecindario(Vecindario) :- forall(restriccionVecindario(RestriccionVecindario), apply(RestriccionVecindario, [Vecindario])).

% Una vez se genero una posible solucion se valida cumpla con todas las demas propiedades %
validarSolucion(Solucion) :-
...
% Restricciones de las propiedades que se saben con certeza de las casas al inicio del problema %
...
% Restricciones de las propiedades que se saben con certeza de las posiciones de las casas al inicio del problema %
